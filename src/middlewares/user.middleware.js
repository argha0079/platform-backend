...
export const syncUser = async (req, res, next) => {
    try {
        ...
        let user = await userService.findUserByClerkId(clerkId);
        if (!user) {
            ...
            const primaryPhoneNumber = clerkUser.phoneNumbers.find(
                (phoneNumber) => phoneNumber.id === primaryPhoneNumberId
            );
            ...
            try {
                ...
                if (error?.code === 'P2002') {
                    ...
                    if (!targetFields.includes('clerkId')) {
                        const conflictError = new Error('Email or phone number already in use');
                        conflictError.statusCode = 409;
                        throw conflictError;
                    }

                    // concurrent creation race on unique clerkId
                    user = await userService.findUserByClerkId(clerkId);
