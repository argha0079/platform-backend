...
const mapDomain = (category) => {
    ...
    return DOMAIN_VALUES.includes(mapped) ? mapped : 'OTHER';
};
...
class ChallengeService {
    ...
    async uploadAndStoreMedia(challengeId, mediaType, file) {
        ...
        if (mediaType === 'IMAGE') {
            uploadResult = await this.cloudinaryService.uploadImage(file.buffer, file.originalname);
        } else {
            uploadResult = await this.cloudinaryService.uploadAudio(file.buffer, file.originalname);
        }
    ...
    async uploadAllMedia(challengeId, imageFile, audioFile) {
        if (imageFile) {
            await this.uploadAndStoreMedia(challengeId, 'IMAGE', imageFile);
        }

        if (audioFile) {
            await this.uploadAndStoreMedia(challengeId, 'AUDIO', audioFile);
        }
    }
    ...
    mapMlResult(mlResult) {
        ...
        const updateData = {
            ...
            mlConfidence: typeof confidence === 'number' ? confidence : null,
            priority: mapPriority(priorityLevel),
            priorityScore: typeof priorityScore === 'number' ? priorityScore : null,
    ...
    async runSimilarityCheck(text, challengeId) {
        const candidates = await this.challengeRepository.findSimilarityTexts(challengeId);
        ...
        if (matchedChallenge) {
            const matchedCandidate = candidates.find(
                (candidate) => sanitizeForSimilarity(candidate.unifiedText) === matchedChallenge
            );
        ...
        return {
            ...
            similarityScore:
                typeof similarity.similarity_score === 'number'
    ...
    async submitChallenge(userId, challengeData) {
        const { title, description, imageFile, audioFile } = challengeData;
        ...
        const hasContent = (description && description.trim()) || imageFile || audioFile;

        if (!hasContent) {
            const error = new Error('Description, image or audio is required');
            error.statusCode = 400;
            throw error;
        }
        ...
        if (mlFailed) {
            ...
            const failedChallenge = await this.challengeRepository.findById(challenge.id);
        ...
        updateData.status = updateData.isDuplicate ? 'DUPLICATE' : 'SUBMITTED';

        await this.challengeRepository.update(challenge.id, updateData);

        // auto-assign to matching organizations by category (non-fatal)
        let assignmentFailed = false;

        if (updateData.status === 'SUBMITTED' && updateData.category) {
            try {
        ...
        const finalChallenge = await this.challengeRepository.findById(challenge.id);

        const warnings = [];

        if (mediaFailed) {
            warnings.push('Media upload failed. Challenge saved without media.');
        }

        if (similarityFailed) {
            warnings.push('Similarity check failed. Challenge submitted without duplicate check.');
        }

        if (assignmentFailed) {
            warnings.push('Auto-assignment to organizations failed.');
        }
    ...
    async adminAssignChallenge(challengeId, organizationId, remarks) {
        const challenge = await this.challengeRepository.findRawById(challengeId);
        ...
        return this.organizationService.manuallyAssign(challengeId, organizationId, remarks);
    }
}
