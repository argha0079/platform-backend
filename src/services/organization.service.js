...
class OrganizationService {
    ...
    async registerOrganization(userId, orgData) {
        const existing = await this.organizationRepository.findByUserId(userId);

        if (existing) {
            const error = new Error('This user is already registered as an organization');
            error.statusCode = 400;
            throw error;
        }

        if (!orgData.name || typeof orgData.name !== 'string' || !orgData.name.trim()) {
        ...
        return this.organizationRepository.findById(organization.id);
    }
    ...
    async updateOrganization(userId, orgData) {
        ...
        return this.organizationRepository.update(organization.id, updateData);
    }
    ...
    async autoAssign(challengeId, category) {
        ...
        const organizations = await this.organizationRepository.findMatching(category);
        ...
        if (assignments.length > 0) {
            await this.organizationRepository.updateChallengeStatus(challengeId, 'ASSIGNED');
        }
    ...
    async listAssignedChallenges(userId) {
        ...
        return this.organizationRepository.findOpenAssignmentsByOrganization(organization.id);
    }
    ...
    async respondToAssignment(userId, assignmentId, response) {
        ...
        const assignment = await this.organizationRepository.findAssignmentById(assignmentId);
        ...
        if (assignment.organizationId !== organization.id) {
            const error = new Error('This assignment does not belong to your organization');
            error.statusCode = 403;
            throw error;
        }

        if (assignment.status !== 'PENDING') {
            const error = new Error('This assignment has already been responded to');
            error.statusCode = 400;
            throw error;
        }
        ...
        const updated = await this.organizationRepository.updateAssignment(assignmentId, {
            status: accept ? 'ACCEPTED' : 'REJECTED',
            respondedAt: new Date(),
        });

        const { challengeId: assignedChallengeId, organizationId: assignedOrganizationId } =
            updated;
        ...
        if (accept) {
            await this.organizationRepository.rejectPendingAssignmentsByChallenge(
                assignedChallengeId,
                assignmentId
            );
            ...
            const remainingPending =
                await this.organizationRepository.countPendingAssignmentsByChallenge(
                    assignedChallengeId
                );

            if (remainingPending === 0) {
                const currentChallenge =
                    await this.organizationRepository.findChallengeStatus(assignedChallengeId);
    ...
    mapOrganizationType(value) {
        ...
        return ORGANIZATION_TYPES.includes(mapped) ? mapped : 'OTHER';
    }
