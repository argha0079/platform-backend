...
class AdminService {
    ...
    async updateUserRole(actorId, userId, role) {
        ...
        if (!role || !ADMIN_ROLE_VALUES.includes(role)) {
            ...
            const error = new Error(
                "role must be one of USER, ORGANIZATION, ADMIN"
            );
    ...
    async updateChallengeStatus(challengeId, status) {
        ...
        return this.adminRepository.updateChallengeStatus(
            challengeId,
            status
        );
    ...
    async verifyOrganization(organizationId, isVerified) {
        ...
        const organization = await this.adminRepository.findOrganizationById(
            organizationId
        );
        ...
        return this.adminRepository.updateOrganization(
            organizationId,
            { isVerified }
        );
