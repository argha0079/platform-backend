...
class ProjectService {
    ...
    async listMilestones(userId, projectId) {
        ...
        return this.projectRepository.findMilestonesByProjectId(
            project.id
        );
    ...
    async updateMilestone(userId, milestoneId, milestoneData) {
        ...
        const milestone = await this.getOwnedMilestone(
            userId,
            milestoneId
        );
        ...
        if (milestoneData.title !== undefined) {
            if (!milestoneData.title || typeof milestoneData.title !== "string" || !milestoneData.title.trim()) {
        ...
        if (milestoneData.dueDate !== undefined) {
            updateData.dueDate = milestoneData.dueDate
                ? new Date(milestoneData.dueDate)
                : null;
        }
        ...
        if (milestoneData.status !== undefined) {
            ...
            const status = this.mapMilestoneStatus(
            ...
            if (status === "COMPLETED") {
                ...
                updateData.completedAt = milestone.completedAt
                    || new Date();
        ...
        return this.projectRepository.updateMilestone(
            milestoneId,
            updateData
        );
    ...
    mapProjectStatus(value) {
        ...
        return PROJECT_STATUS_VALUES.includes(mapped)
            ? mapped
            : "NOT_STARTED";
    ...
    mapMilestoneStatus(value) {
        ...
        return MILESTONE_STATUS_VALUES.includes(mapped)
            ? mapped
            : "PENDING";
