...
class AdminRepository {
    async getStats() {
        ...
        const countsFrom = (rows, key) =>
            Object.fromEntries(
                rows.filter((row) => row[key]).map((row) => [row[key], row._count._all])
            );
        ...
        return {
            ...
            totalAssignments,
            totalMilestones,
            challengesByStatus: countsFrom(challengesByStatus, 'status'),
            challengesByPriority: countsFrom(challengesByPriority, 'priority'),
            challengesByCategory: countsFrom(challengesByCategory, 'category'),
        };
    }
